...
import { $isCaptionEditorEmpty, $isImageNode } from './EditorImageNode';
...
function LazyImage({
  ...
  const calculateDimensions = () => {
    ...
    const naturalWidth = (status as { width: number }).width;
    const naturalHeight = (status as { height: number }).height;
...
export default function ImageComponent({
  ...
  // const { isCollabActive } = useCollaborationContext(); //协助
  const [editor] = useLexicalComposerContext();
  ...
  return (
    <Suspense fallback={null}>
      {/* 块状元素默认占领整行，需要调整为display: inline-block */}
